...
impl ShaderArea {
    pub fn initialize(&self, shader: String, textures: Vec<PathBuf>) {
        INIT_EPOXY.call_once(init_epoxy);
...
"
            ...
            unsafe {
                ...
                for (i, tex) in textures.iter().enumerate() {
                    let Some(id) = load_texture(i as u32, tex) else {
                        continue;
                    };
                    texture_ids.push(id);
...
unsafe fn link_program(vertex: &str, fragment: &str) -> u32 {
    let vert = compile_shader(vertex, epoxy::VERTEX_SHADER);
    let frag = compile_shader(fragment, epoxy::FRAGMENT_SHADER);
